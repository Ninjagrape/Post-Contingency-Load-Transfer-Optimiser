...
```
=== SUBSTATION OUTAGE OFFLOAD PLAN v2  (season: summer, scenario: baseline) ===
Outage: SUB-X
...
Switching effort: 1 open + 2 close (1 remote, 2 manual)
```

## Network model

The built-in network is defined in `scenarios/baseline.json` and represents three substations with a complex mix of feeder topologies:
